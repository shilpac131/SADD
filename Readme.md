...
For GPU training
  - GPU: 1 NVIDIA Tesla 2080Ti
    - About 12GB is required to train the networks using a batch size of 16 & 24 for sSADD and SADD respectively.
...
This network uses speaker information captured from the LP residual. To capture information from the LP residual, we utilize a multi-scale convolution-based architecture with varying kernel sizes, followed by a transformer encoder layer for capturing long range dependencies.
...
Run the following command to train the sSADD network.
```bash
